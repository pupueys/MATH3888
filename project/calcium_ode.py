...
def dCER_dt(t, C, C_ER, V, P):
    ...
    I_IP3R_chan  = I_IP3R(C, C_ER, P, V, V)      # IP3R current (per channel)
...
def calcium_ode (t, y):
    ...
    # channel currents 
    I_PMCA_chan  = bar_I_PMCA * g_PMCA                  # current per PMCA channel (pA)
    I_CRAC_chan  = I_CRAC(V, C)                         # current per CRAC         (pA)
    I_SERCA_chan = I_SERCA(C)                           # current per SERCA        (pA)
    I_IP3R_chan  = I_IP3R(C, C_ER, P, V, V)             # IP3R current             (pA)

    dVdt = -(I_CRAC_chan + I_PMCA_chan + I_SERCA_chan + I_IP3R_chan) * 1000 / (C_m * A_cell)   
...
# Only print every 10 seconds
    if int(t) % 10 == 0:
        print(f"t={t:.1f}s | C={C:.3f} µM | C_ER={C_ER:.1f} µM | P={P:.3f} µM | "
          ...
          f"I_SERCA={I_SERCA_chan:.3e} | I_PMCA={I_PMCA_chan:.3e} | V={V:.3f}")
